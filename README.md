...
```bash
# Register Fibe with your MCP client (claude-code | claude-desktop | cursor | vscode)
...
The server registers ~100 tools that map 1:1 to CLI leaf commands — `fibe_playgrounds_list`, `fibe_playgrounds_get`, `fibe_launch`, `fibe_tricks_trigger`, etc. Input schemas are derived from the SDK's `*Params` structs so agents get type-checked arguments. Set `FIBE_MCP_TOOLS=core` (default, ~40 tools) or `FIBE_MCP_TOOLS=full` to control the advertised surface.
