...
func TestCLI_E2E_Commands(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			createCmdName: "create",
			updateCmdName: "update",
			createArgs:    []string{"--url", "https://sdk-cli-webhook.invalid/hook", "--event", "artefact.created", "--secret", "webhook_secret_key_123"},
			updateArgs:    []string{"--url", "https://sdk-cli-webhook.invalid/hook2"},
